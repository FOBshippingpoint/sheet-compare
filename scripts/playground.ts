...
import { runDiffRows } from "../src/lib/diff";
import { loadTableFile } from "../src/lib/files";
...
if (command === "sheet") {
  ...
  if (!leftPath || !rightPath) throw new Error("Missing left or right file path.");
...
function parseFlags(flags: string[]): PlaygroundFlags {
  const options: PlaygroundFlags = {
    leftSheet: undefined,

...
import { mountComponent } from "./helpers.js";
...
test("frozen header remains mounted and positioned during vertical scroll", async () => {
  ...
  const headerCell = scrollport.querySelector('[data-row-index="0"][data-column-index="1"]');
  const initialTop = Math.round(headerCell.getBoundingClientRect().top);
  const removedHeaderCells = [];
  ...
  for (let step = 1; step <= 20; step++) {
    ...
    const currentHeaderCell = scrollport.querySelector('[data-row-index="0"][data-column-index="1"]');
...
test("frozen column does not drift during smooth horizontal scroll", async () => {
  ...
  const scrollport = page.getByLabelText("Standalone frozen table").element();
  const columnCell = scrollport.querySelector('[data-row-index="1"][data-column-index="0"]');
...
test("frozen corner remains mounted and positioned during horizontal scroll", async () => {
  ...
  const scrollport = page.getByLabelText("Standalone frozen table").element();
  const cornerCell = scrollport.querySelector('[data-row-index="0"][data-column-index="0"]');
  ...
  for (let step = 1; step <= 20; step++) {
    ...
    const currentCornerCell = scrollport.querySelector('[data-row-index="0"][data-column-index="0"]');
...
function watchRemovedHeaderCells(scrollport, removedHeaderCells) {
  const canvas = scrollport.querySelector(".virtual-canvas");
...
function cell(scrollport, rowIndex, columnIndex) {
  return scrollport.querySelector(`[data-row-index="${rowIndex}"][data-column-index="${columnIndex}"]`);
}

async function samplePlacement(read, count) {
  const samples = [];

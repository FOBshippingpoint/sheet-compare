...
import { mountApp, requireElement } from "./helpers";
...
test("preview table shows spreadsheet labels and freeze controls", async () => {
  ...
  const leftPreview = requireElement(
    page.getByRole("heading", { name: "Left" }).element().closest(".preview"),
  );
  const handle = requireElement(
    leftPreview.querySelector('button[aria-label^="Adjust frozen rows and columns"]'),
  );
...
test("diff table keeps frozen cells while scrolling", async () => {
  ...
  const actionCell = requireElement(diffPane.querySelector(".action-cell"));
...
test("large previews render a bounded cell set while scrolling", async () => {
  ...
  const previewScrollport = requireElement(
    requireElement(
      page.getByRole("heading", { name: "Left" }).element().closest(".preview"),
    ).querySelector<HTMLElement>('[aria-label="Sheet preview table"]'),
  );
